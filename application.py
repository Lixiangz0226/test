...
def process_event_history(log: dict[str, list[dict]],
    ...
    for event_data in log['events']:
        ...
        if event_data["type"] == "call":
            ...
            new_call = Call(src_number, dst_number, time,
                            duration, src_loc, dst_loc)

            src_customer = find_customer_by_number(src_number, customer_list)
            src_customer.make_call(new_call)

            dst_customer = find_customer_by_number(dst_number, customer_list)
