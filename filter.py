...
class CustomerFilter(Filter):
    ...
    def apply(self, customers: list[Customer],
        ...
        """
        if filter_string is None or len(filter_string) != 4 or not \
                filter_string.isnumeric():
            return data

        customer_id = int(filter_string)
        customer_from_id = None
        for customer in customers:
            if customer.get_id() == customer_id:
                customer_from_id = customer
        if customer_from_id is None:
            return data

        customer_phone_numbers = customer_from_id.get_phone_numbers()
        filtered_data = []
        for call in data:
            if call.src_number in customer_phone_numbers:
                filtered_data.append(call)
            elif call.dst_number in customer_phone_numbers:
                filtered_data.append(call)
...
class DurationFilter(Filter):
    ...
    def apply(self, customers: list[Customer],
        ...
        Do not mutate any of the function arguments!
        """
        if filter_string is None or len(filter_string) != 4 or \
                (filter_string[0] != 'L' and filter_string[0] != 'G') \
                or not filter_string[1:4].isnumeric():
            return data
...
class LocationFilter(Filter):
    ...
    def apply(self, customers: list[Customer],
        ...
        if filter_string is None:
            return data
        split_coordinates_str = filter_string.strip().split(", ")
        if len(split_coordinates_str) != 4:
            return data

        coordinates = []
        for str_coordinate in split_coordinates_str:
            coordinate = self.str_to_float(str_coordinate)
            if coordinate == "":
                return data
        ...
        lowerLong = coordinates[0]
        lowerLat = coordinates[1]
        upperLong = coordinates[2]
        upperLat = coordinates[3]
        filtered_data = []

        for call in data:
            if lowerLong <= call.dst_loc[0] <= upperLong and lowerLat \
                    <= call.dst_loc[1] <= upperLat:
                filtered_data.append(call)
            elif lowerLong <= call.src_loc[0] <= upperLong and lowerLat \
                    <= call.src_loc[1] <= upperLat:
                filtered_data.append(call)
    ...
    def str_to_float(self, str_filter: str) -> Union[str, float]:
        """
        check if the str is a valid decimal number. return an empty string
        if the specifications aren't met.Else return the str as a float.
        """
        if str_filter is None:
            return ""
        if "." not in str_filter:
            return ""
        decimal = str_filter.index(".")
        if str_filter[0] == "-":
            if not str_filter[1:decimal].isnumeric() or not \
                    str_filter[decimal + 1:].isnumeric():
                return ""
            float_filter = float("-" + str_filter[1:decimal] + "."
                                 + str_filter[decimal + 1:])
        else:
            if not str_filter[:decimal].isnumeric() or not \
                    str_filter[decimal + 1:].isnumeric():
                return ""
            float_filter = float(str_filter[:decimal] + "."
                                 + str_filter[decimal + 1:])
        return float_filter
